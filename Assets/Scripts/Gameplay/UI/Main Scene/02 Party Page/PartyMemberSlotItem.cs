...
namespace Mathlife.ProjectL.Gameplay
{
    public class PartyMemberSlotItem : Presenter<int>
    {
        ...
        {
            m_dragItem = gameDataLoader.Instantiate<PartyMemberSlotDragItem>(EPrefabId.PartyMemberSlotDragItem, m_dragItemParent);
            m_dragItem.Initialize(m_character);
            m_dragItem.transform.position = transform.position;

            m_canvasGroup.HideWithAlpha(0.25f);

            m_partyPage.isDraggingSlotItemRx.Value = true;
        }
        ...
        {
            m_partyPage.isDraggingSlotItemRx.Value = false;
