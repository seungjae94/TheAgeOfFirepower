...
namespace Mathlife.ProjectL.Gameplay
{
    ...
    /// </example>
    public abstract class GameMode<TGameMode> : MonoSingleton<TGameMode> where TGameMode : GameMode<TGameMode>
    {
        private void Start()
        {
            Initialize().Forget();
        ...
        private async UniTaskVoid Initialize()
        {
            ...
            if (GameManager.Inst.IsGameInitialized == false)
            {
                await PreInitializeGame();
                await GameManager.Inst.InitializeGame();
                await PostInitializeGame();
            }

            IProgress<float> progress = Progress.Create<float>(value => LoadingScreenManager.Inst.SetProgress(value));
            await InitializeScene(progress);
            
            LoadingScreenManager.Inst.Hide();
        }

        protected virtual UniTask PreInitializeGame()
        {
            return UniTask.CompletedTask;
        }
        
        protected virtual UniTask PostInitializeGame()
        {
            return UniTask.CompletedTask;
        }

        protected abstract UniTask InitializeScene(IProgress<float> progress);
    }
}